# -*- coding: utf-8 -*-
"""
Created on Fri Sep 14 10:31:21 2018
...
data = pd.read_csv("data/comments1.tsv", header=0, \
                    delimiter="\t", quoting=3)



data = data[['Comments','Ispositive']]
data1 = data[['Comments','Ispositive']]



# Veri Setinin temizlenmesi.
data['Comments'] = data['Comments'].apply(lambda x: x.lower())
data2 = data[['Comments','Ispositive']]

data['Comments'] = data['Comments'].apply((lambda x: re.sub('[^'+turkish_characters+'\s]','',x)))
data3 = data[['Comments','Ispositive']]   
...
X1 = tokenizer.texts_to_sequences(data['Comments'].values)
...
Y = pd.get_dummies(data['Ispositive']).values
...
# Save Model
model.save("models/SentenceEvaluation.h5")
...
model = load_model("models/SentenceEvaluation.h5")


my_text = ["güzel bir tavsiye ederim",
